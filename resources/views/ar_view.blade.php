...
<head>
    ...
    <style>
        /* ESTILOS INTERFAZ UI */
        body { margin: 0; overflow: hidden; font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; touch-action: none; user-select: none; -webkit-user-select: none; -webkit-tap-highlight-color: transparent; }
        .a-enter-vr, .a-enter-vr-button { display: none !important; }
        .arjs-video { width: 100vw !important; height: 100vh !important; position: absolute !important; top: 0 !important; left: 0 !important; object-fit: cover !important; z-index: -1 !important; }
        
        #top-bar { position: absolute; top: 0; left: 0; width: 100%; height: 70px; pointer-events: none; display: flex; justify-content: space-between; align-items: flex-end; padding: 10px 15px; box-sizing: border-box; z-index: 9999; }
        .btn-pill { pointer-events: auto; background: rgba(20, 20, 20, 0.6); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); color: white; border: 1px solid rgba(255,255,255,0.2); padding: 8px 16px; border-radius: 20px; font-weight: 600; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; display: flex; align-items: center; gap: 5px; box-shadow: 0 4px 6px rgba(0,0,0,0.2); transition: transform 0.1s; cursor: pointer; }
        ...
        #ui-layer { position: absolute; bottom: 0; left: 0; width: 100%; padding: 10px 15px 30px 15px; box-sizing: border-box; z-index: 9999; pointer-events: none; display: flex; justify-content: space-between; align-items: flex-end; }
        .joystick-area { pointer-events: auto; position: relative; width: 38vw; height: 38vw; max-width: 160px; max-height: 160px; min-width: 120px; min-height: 120px; background: rgba(0, 0, 0, 0.3); border-radius: 50%; backdrop-filter: blur(5px); border: 1px solid rgba(255,255,255,0.1); }
        .d-pad-grid { width: 100%; height: 100%; display: grid; grid-template-columns: 1fr 1fr 1fr; grid-template-rows: 1fr 1fr 1fr; }
        .d-btn { display: flex; align-items: center; justify-content: center; color: rgba(255,255,255,0.8); font-size: 24px; cursor: pointer; border-radius: 50%; }
        .d-btn:active { background: rgba(255,255,255,0.3); color: white; }
        .cell-up { grid-column: 2; grid-row: 1; } .cell-down { grid-column: 2; grid-row: 3; } .cell-left { grid-column: 1; grid-row: 2; } .cell-right { grid-column: 3; grid-row: 2; }
        .cell-center { grid-column: 2; grid-row: 2; font-size: 10px; font-weight: bold; color: rgba(255,255,255,0.4); pointer-events: none; text-align: center; display: flex; align-items: center; }
        .center-stack { pointer-events: auto; display: flex; flex-direction: column; gap: 10px; margin-bottom: 10px; }
        .control-pair { display: flex; flex-direction: column; align-items: center; background: rgba(0,0,0,0.5); padding: 5px; border-radius: 20px; backdrop-filter: blur(10px); }
        .mini-label { font-size: 9px; color: #aaa; font-weight: 700; margin-bottom: 4px; letter-spacing: 1px;}
        .btn-row { display: flex; gap: 2px; }
        .btn-sq { width: 40px; height: 35px; background: rgba(255,255,255,0.1); border: none; color: white; font-size: 16px; font-weight: bold; display: flex; align-items: center; justify-content: center; cursor: pointer; }
        ...
        @media (max-width: 360px) { .joystick-area { width: 35vw; height: 35vw; } .btn-sq { width: 30px; } .d-btn { font-size: 20px; } #ui-layer { padding-bottom: 10px; } }
    </style>
</head>
